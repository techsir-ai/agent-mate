...
### 开发设想文档升格为需求文档并改名(老板指令 A)

**指令**:方案 A——将「开发设想与路线」改名为需求文档,改名后删除之前备份文件,做 commit。

**动作**:
1. `git mv agent-mate-开发设想与路线.md → agent-mate需求文档.md`,保留 git 历史。
2. 文档内部标题改为 `# Agent Mate — 需求文档（v0.01）`。
3. 删除备份文件 `agent-mate-开发设想与路线.md.001.md`、`.002.md`、`.003.md`(受 .gitignore 保护,本就不进版本库;现按要求删除本地副本)。
4. 定位:依据「需求文档 = 功能清单,实现即完成」的判断,此文档已具备 v0.01 需求基线资格,不改写、不补验收标准(验收属实现/Spec 阶段)。

**版本号**:前置文档升格,开发未开始,保持 v0.00.000。

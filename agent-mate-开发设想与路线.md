...
- **支持程度**:`pwsh`(PowerShell 7,首选)> Windows PowerShell 5.1(可回退)> `cmd`(不支持)。
  - pwsh:功能完整。
  - Windows PowerShell 5.1:作回退,有局限、可能出错;仍不支持 cmd,命令需兼容 5.1 语法。
  - **cmd 不支持**(所有 agent 都不支持 cmd,功能缺失过多无法补丁)。
- **非 dsh 硬前置**:pwsh 硬依赖 ssh;仅当 ssh 部署需要时才真正装机。
